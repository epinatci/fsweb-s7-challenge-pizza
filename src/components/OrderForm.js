...
const OrderForm = () => {
    ...
    const ekMalzemeler = ["Pepperoni", "Domates", "Biber", "Sosis", "Mısır", "Sucuk", "Kanada Jambonu", "Ananas", "Tavuk Izgara", "Jalepeno", "Kabak", "Soğan", "Sarımsak"]
    ...
    const handleSubmit = (e) => {
        ...
        axios
            ...
            .then(res => {
                ...
                history.push({ pathname: '/confirm', state: formData, pizza: pizzaBilgileri });
    ...
    const calculateOrderTotal = (formData) => {
        ...
        const toplamFiyat = (pizzaBilgileri.fiyat + calculateSelectedItemsTotal(formData)) * formData.adet;
        if (formData.boyut === "S") {
            result = toplamFiyat
        ...
        <div className='order-page'>
            ...
            <div className='contents'>
                ...
                <nav>
                    ...
                    <a href="/orderform" style={{ color: "#5F5F5F" }}> - Seçenekler</a>
                    <a href="/confirm" style={{ color: "#CE2829" }}> - Sipariş Oluştur</a>
                </nav>

                <h3 style={{ width: "40%", margin: "auto", marginBottom: "3%" }}>{pizzaBilgileri.isim}</h3>

                <div style={{ width: "40%", margin: "auto", display: "flex", marginBottom: "3%" }}>

                    <h2 style={{ width: "75%" }}>{pizzaBilgileri.fiyat + "₺"}</h2>
                    <p style={{ color: "#5F5F5F", width: "15%" }}>{pizzaBilgileri.begeni}</p>
                    <p style={{ color: "#5F5F5F", width: "10%" }}>{pizzaBilgileri.begenenSayisi}</p>

                </div>

                <p style={{ width: "40%", margin: "auto", marginBottom: "3%", color: "#5F5F5F" }}>{pizzaBilgileri.text}</p>
            </div>
            <div className='order-form-container' >
                <form id="pizza-form" onSubmit={handleSubmit}>
                    <div className='boyut-hamur'>
                        <div className='pizza-boyutu'>
                            ...
                            <input
                                ...
                                required
                            />
                            <label style={{ marginTop: "4%" }} htmlFor="boyut-kucuk">S</label>
                            ...
                            <p style={{ color: "#CE2829", fontSize: "0.8rem", fontFamily: "Quattrocento", margin: "0", marginTop: "2%" }}>{formErrs.boyut}</p>
                        </div>

                        <div className='hamur'>
                            <h5>Hamur Seç<span style={{ color: "#CE2829" }}>*</span></h5>

                            <select id="hamur" name="hamur" value={formData.hamur} onChange={handleChange} data-cy="hamur" style={{ marginTop: "4%" }} required>
                            ...
                            <p style={{ color: "#CE2829", fontSize: "0.8rem", fontFamily: "Quattrocento", margin: "0", marginTop: "2%" }}>{formErrs.hamur}</p>
                        </div>
                    </div>
                    ...
                    <div className='siparis-notu'>
                        ...
                        <input type="text" id="siparisNotu" name="siparisNotu" data-cy="note-input" value={formData.siparisNotu} onChange={handleChange} placeholder='Siparişinize eklemek istediğniz bir not var mı?' />
                    </div>
                    <div className='adet-toplam'>
                        <div className='adet-button'>
                            <label htmlFor="adet">Adet:</label>
                            <button type="button" onClick={handleAdetEksilt} style={{ margin: "0" }}>-</button>
                            <input type="number" id="adet" name="adet" data-cy="adet" value={formData.adet} onChange={handleChange} min="1" />
                            <button type="button" onClick={handleAdetArttir} style={{ margin: "0" }}>+</button>
                        </div>
                        ...
                        <div className="siparis-tutari">
                            ...
                            <div className='fiyat'>
                                ...
                                <p style={{ color: "#CE2829" }}>{calculateOrderTotal(formData)} ₺</p>
                            </div>
                            <button id='order-button' type="submit" data-cy="submit-button" disabled={!isValid} style={{ margin: "0" }}>Sipariş Ver</button>
                        </div>
                    </div>
                </form>
            </div>
